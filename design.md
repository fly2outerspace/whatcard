...
> **[评注 — 步数上限计算方式，已实现]** 旧设计曾使用 `movesLimit = ceil(appliedMoves × 2.0) + 8`，其中 2.0 是模糊倍率。根本原因是 `t2t` 反向操作可以整体移动 N 张牌（1 步），但正向游戏受 `faceUp` 约束，这 N 张牌初始背面朝下，玩家必须逐张翻开移动，实际耗费 N 步。
>
> **当前方案**：倒推每一步精确累加其对应的前向代价：
> - `f2t` / `f2s` / `t2s`：各计 **1** 步（均对应 1 次正向操作）
> - `t2t`（移动 groupSize 张）：计 **groupSize** 步（每张牌因 faceUp 需单独操作）
>
> `preciseCost = Σ 上述代价`（= 考虑了 faceUp 后的理论最少步数）
>
> `movesLimit = preciseCost + moveBuffer`（`moveBuffer` 为固定整数，给玩家一定容错余量）
>
> 见 `src/game/LevelGenerator.ts`，`GenerationStats.preciseCost`。
...
> 3. 重试次数不确定，调参困难
>
> **当前策略**：可解性由**倒推生成法在生成阶段直接保证**，Bot 仅用于事后的**难度标定**（测通关率），不参与可解性验证。
