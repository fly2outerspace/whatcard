...
> **[评注 — 卡牌翻开状态]** 每张卡牌有 `faceUp`（翻开/背面）状态，这是核心机制：
> - 初始发牌后，每叠 Tableau **仅顶牌翻开**，其余全部背面朝下
> - 玩家只能操作已翻开的牌；背面朝下的牌不可见、不可移动
> - 顶部牌被移走后，下一张自动翻开
> - 即使两张同类别牌相邻堆叠，若下方那张背面朝下，它们**不会自动合并**，玩家必须先移走上方的牌使其翻开，才能进行操作
> - Stock 中的牌全部背面朝下，抽出后翻开进入 Discard
...
> **[评注 — 本章定位]** 本章（§2.1–2.3）是早期阶段对可解性问题的理论建模，作为背景知识保留。**生产中不实现 `DifficultyAnalyzer`，也不用运行时验证可解性。** 可解性由倒推生成法在生成阶段直接保证（§3.1），依赖图分析不再作为游戏流程的组成部分。
...
> **[评注 — 倒推生成法的修正]** 原设计中倒推操作的 `f2t`（Foundation→Tableau）沿用了正向游戏的同类别约束，导致生成的 Tableau 全部是纯同类别堆叠，游戏可玩性丧失。
>
> **根本原因**：倒推操作需要区分两类行为：
> - `t2t`（叠间移动）= 正向游戏操作的逆转，保留同类别约束
> - `f2t`（Foundation→Tableau）= 初始发牌的逆转，初始发牌不受游戏规则约束，**应允许放到任意叠**（包括不同类别顶牌、基座牌上方）
>
> **修正后**：`f2t` 无类别约束，可放到任意叠，模拟随机初始发牌，产生自然的跨类别混叠效果。
...
### 3.1 当前主线：前向发牌（Shuffle → Deal）

Phase 3 起，关卡生成采用「**Shuffle → Deal**」的前向发牌：先洗牌，再按 Tableau 梯形深度发牌（左→右递增高度），剩余进入 Stock。贪心 Bot 保留，用于标定合理的总步数上限（`movesLimit`）。

**生成流程**：

```
1. 按类别与每类牌数构造完整牌堆（含基座牌）
2. 使用 seed（可选）对牌堆做随机洗牌（shuffle）
3. 计算 tableau 目标牌数：
     tableauTotal = floor(totalCards × 18/64)
   并将其分配为 4 叠非递减梯形（右叠 ≥ 左叠）
4. 从洗牌后的牌堆顶部依次发到 Tableau（左→右），剩余全部进入 Stock
```

**为什么选这条路**：
- 更贴近玩家直觉与“洗开”的观感（桌面先成型、Stock 作为剩余牌堆）
- 更适合“旅程式关卡”的工作流：通过 seed + 参数手动调参得到满意布局
- 工程复杂度更低，避免倒推在本项目规则/约束下引入的偏差与额外语义（详见下方评注）

> **[评注 — 已废弃方案：倒推生成法（Backward Generation）]** 旧主线曾以倒推生成作为“保证可解”的方法：
>
> ```
> 1. 从「胜利状态」出发
> 2. 执行 N 步反向操作，将牌放回 Tableau 或 Stock
> 3. 反向 N 步后的局面作为初始局
> 4. 调整 N 与参数旋钮控制难度
> ```
>
> **优点**：生成快、参数直观、可解性在简化假设下容易证明。  
> **问题**：在本项目真实约束（Stock/Discard 两步代价、槽位调度、隐藏信息与玩家体验）下会引入分布偏差与工程复杂度，并不等价于“随机洗牌后发牌”的体验，因此降级为评注历史记录。

### 3.2 难度参数旋钮

当前旋钮以“可手工调参、可被 Bot 标定”为目标：

| 旋钮 | 作用 | 备注 |
|------|------|------|
| 类别数量 | 槽位调度压力与信息复杂度 | 通常 ≥ 8 更有挑战 |
| 每类牌数 | 总牌数与堆叠深度 | 决定信息密度与抽牌压力 |
| seed | 复现同一布局 | 便于调参与固化关卡 |
| `movesLimit`（总步数上限） | 成功/失败窗口 | 通过贪心 Bot ×100 标定 |

> **[评注 — 已废弃旋钮表（倒推时代）]** 旧方案用 `movesBack/moveBuffer` 等参数驱动生成与步数推导；该表保留仅作历史对照。
>
> | 旋钮 | 简单 | 中等 | 困难 |
> |------|------|------|------|
> | 反向步数 N（movesBack） | 低（布局接近有序） | 中 | 高（布局高度混乱） |
> | 基座牌埋藏深度 | 顶部或 Stock 前段 | 中段 | 底部或 Stock 后段 |
> | 类别间交叉程度 | 同类牌集中在同叠 | 适度分散 | 高度混叠 |
> | 步数缓冲（moveBuffer） | 大（容错多） | 中 | 小（几乎无冗余） |
...
当前主线（Shuffle→Deal）下，Bot 的首要用途是**标定 `movesLimit`**：在固定的“类别数/每类牌数/seed/movesLimit”下批量跑 100 局，得到通关率与用步统计，用于调参和固化关卡。
...
> **[评注 — 已废弃表述（倒推时代）]** 旧文将“可解性由倒推生成保证”作为前提，并把 Bot 视为“难度标定的附属工具”。Phase 3 切换后不再使用该前提：生成本身是 shuffle+deal；Bot 的核心用途是 movesLimit 标定与参数对比。
>
> **[评注 — 方向B已放弃]** 原设计中曾考虑加入「最优 Bot（A* 搜索）→ 必须通关 → 确认关卡存在解」的一行。这属于**方向B思路**（随机生成 + Bot 验证可解性 + 不可解则重试），已决定放弃，原因：
> 1. 可解性判断本身是 NP 问题；贪心 Bot 失败 ≠ 真不可解，A* 状态空间爆炸
> 2. 无法追溯解路径，movesLimit 设置失去依据
> 3. 重试次数不确定，调参困难
>
> **当前策略**：生成使用 shuffle+deal；Bot 用于标定 movesLimit 与参数梯度；关卡固化依赖 seed 与手工筛选，而非“生成即保证可解”。
...
│   # DifficultyAnalyzer.ts ← 已移除：依赖图分析不作为生产工具实现，见§2评注
│
├── ui/                      # 渲染与交互层
