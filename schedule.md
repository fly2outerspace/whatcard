...
- [x] **tableau 堆叠可读性优化（含拖动一致性）**：已翻开但被压在下方的牌，在上缘可见区域显示 `card-peek` 标签；被覆盖时隐藏中央大字。`DragHandler.buildGhost` 对幽灵牌叠同步 `is-covered`，修复拖动同类牌组时文字回跳到中间的问题。
- [x] **卡牌信息精简**：移除卡牌下方灰色分类小字（`.card-category`），仅保留主标签与必要的覆盖态提示标签。
- [x] **foundation 信息表达升级**：当槽内只有基座牌时保持原视觉；有同类牌叠加后，显示顶部牌并露出基座的书签式 badge（支持任意业务字段，不预设字母），同时在顶部牌右上角显示进度 `已收集/总需求`（例如 `1/5`，动态计算）。
- [x] **stock 抽牌翻面动效升级**：实现“右到左翻出 + 中间白线”效果；抽牌动画压缩为极快节奏（近一帧观感），翻面与位移同步。
- [x] **discard 展示规则重做**：弃牌区显示最近 3 张横向叠牌（最新在最左、最上层）；新牌进入时最旧可见牌移除，其余右移并由新牌覆盖；下层两张显示右侧旋转标签（头朝右）提升可读性。
- [x] **stock / discard 细节表现补充**：stock 右侧牌堆新增“厚度层叠”视觉（最多 4 层，库存降到 3/2/1 时厚度逐级递减）；空弃牌区去掉“弃牌”占位文案，保持纯净背景。
- [x] **移动端布局与尺寸统一调整（合并项）**：卡牌整体放大并最终定版为约原始 `1.5x`（含 JS 常量同步）；顶部布局改为三行结构：第 1 行菜单+步数，第 2 行右侧 stock/discard，第 3 行 foundation 与 tableau，避免互相遮挡。
- [x] **Bugfix：stock 用尽后回收牌序**：`reshuffleDiscard` 原先对弃牌堆 `reverse()` 再写入 stock，导致新一轮抽牌顺序与关卡初始 `level.stock` 相反。弃牌数组顺序本就与当初抽牌顺序一致（`shift` + `push`），应**按弃牌顺序原样**写回 stock（`index 0` = 下一张要抽）。修复位置：`GameState.ts` → `reshuffleDiscard`。
- [x] **Tableau 拖动 / 点选体验**：可从**可移动组内任意一张**已翻开牌开始拖动（不再仅限最顶一张）；幽灵牌定位按抓取深度对齐，避免手型跳变。落点检测改为以**牌组最底一张牌的中心**为探测点，便于用底部对齐基座槽。`ClickHandler` 点选规则与拖动一致。
